    ...
    <div className="min-w-[250px] flex-1">
      ...
      {cpvSearch && (
        <div className="max-h-40 overflow-y-auto border border-slate-200 rounded-lg bg-white shadow mb-2 z-10 relative">
          {filteredCpvs.length === 0 && (
            <div className="p-2 text-slate-700">No hay resultados</div>
          )}
      ...
      <div className="flex flex-wrap gap-2 mt-2">
        {selectedCpvs.map(code => (
          <span key={code} title={cpvMap[code]} className="bg-slate-200 text-slate-700 px-3 py-1 rounded-full text-xs flex items-center">
